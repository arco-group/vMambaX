...
from src.models.encoders.vmamba import CVSSDecoderBlock
...
class UpsampleExpand(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.interpolate(x, scale_factor=2, mode='bilinear', align_corners=False).permute(
            0, 2, 3, 1).contiguous()  # B, 2H, 2W, C/2
        x = self.norm(x)
        return x
...
class FinalUpsample_X4(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.interpolate(x, scale_factor=2, mode='bilinear', align_corners=False).permute(
            0, 2, 3, 1).contiguous()  # B, 2H, 2W, C
        x = self.linear2(x).permute(0, 3, 1, 2).contiguous()  # B, C, 2H, 2W
        x = F.interpolate(x, scale_factor=2, mode='bilinear', align_corners=False).permute(
            0, 2, 3, 1).contiguous()  # B, 4H, 4W, C
        x = self.norm(x)
        return x
...
class CrossAttentionFusion(nn.Module):
    def __init__(self, dim: int, num_heads: int = 4, dropout: float = 0.0) -> None:
        super().__init__()
        self.num_heads = self._resolve_heads(dim, num_heads)
        self.norm_q = nn.LayerNorm(dim)
        self.norm_kv = nn.LayerNorm(dim)
        self.attn = nn.MultiheadAttention(
            embed_dim=dim, num_heads=self.num_heads, dropout=dropout, batch_first=True)
    ...
    def forward(self, up_feat: torch.Tensor, skip_feat: torch.Tensor) -> torch.Tensor:
        """
        Args:
            up_feat: B x H x W x C, upsampled decoder representation (queries)
            skip_feat: B x H x W x C, encoder skip features (keys/values)
        """
        B, H, W, C = up_feat.shape
        up_seq = up_feat.view(B, H * W, C)
        skip_seq = skip_feat.view(B, H * W, C)

        q = self.norm_q(up_seq)
        kv = self.norm_kv(skip_seq)

        attn_out, _ = self.attn(q, kv, kv, need_weights=False)
        attn_out = self.attn_proj(attn_out)
        fused = up_seq + attn_out
        fused = fused + self.ffn(fused)

        return fused.view(B, H, W, C)
...
class MambaDecoder(nn.Module):
    def __init__(self,
        ...
        if self.deep_supervision:
            self.norm_ds = nn.ModuleList([norm_layer(
                embed_dim * 2 ** (self.num_layers - 2 - i_layer)) for i_layer in range(self.num_layers-1)])
            self.output_ds = nn.ModuleList([nn.Conv2d(in_channels=embed_dim * 2 ** (self.num_layers - 2 - i_layer),
                                           out_channels=self.num_classes, kernel_size=1, bias=False) for i_layer in range(self.num_layers-1)])
        ...
        if not self.deep_supervision:
            for inx, layer_up in enumerate(self.layers_up):
                if inx == 0:
                    ...
                    y = F.interpolate(y.permute(0, 3, 1, 2).contiguous(), size=(
                        H, W), mode='bilinear', align_corners=False).permute(0, 2, 3, 1).contiguous()

                    skip = inputs[3 - inx].permute(0, 2, 3, 1).contiguous()
                    x = y + skip
                    y = layer_up(x)
    ...
    def forward(self, inputs):
        if not self.deep_supervision:
            ...
            x_last = self.up_x4(x, self.patch_size)
            x_output_0 = self.output_ds[0](F.interpolate(x_upsample[0].permute(
                0, 3, 1, 2).contiguous(), scale_factor=16, mode='bilinear', align_corners=False))
            x_output_1 = self.output_ds[1](F.interpolate(x_upsample[1].permute(
                0, 3, 1, 2).contiguous(), scale_factor=8, mode='bilinear', align_corners=False))
            x_output_2 = self.output_ds[2](F.interpolate(x_upsample[2].permute(
                0, 3, 1, 2).contiguous(), scale_factor=4, mode='bilinear', align_corners=False))
