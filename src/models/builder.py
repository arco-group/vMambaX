...
from src.utils.init_func import init_weight
...
class EncoderDecoder(nn.Module):
    ...
    def encode_decode(self, rgb, modal_x):
        ...
        if not self.deep_supervision:
            ...
            x = self.backbone(rgb, modal_x)
            x_last, x_output_0, x_output_1, x_output_2 = self.decode_head.forward(
                x)
    ...
    def forward(self, rgb, modal_x, label=None):
        if not self.deep_supervision:
            ...
            x_last, x_output_0, x_output_1, x_output_2 = self.encode_decode(
                rgb, modal_x)
...
def selective_scan_flop_jit(inputs, outputs):
    ...
    flops = flops_selective_scan_fn(
        B=B, L=L, D=D, N=N, with_D=True, with_Z=False)
    return flops
